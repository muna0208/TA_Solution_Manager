	...
	$.jstree.core.prototype = {
      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			var model = $("#jstree_demo").jstree(true)._model.data;
			var newNode = "New node_";
			var nodeList = [];
			var text = "";
			$.each(model, function(idx, idata){
				if( idx != "#" ){
					text = idata.text;
					if( text.indexOf(newNode) != -1 ){
						nodeList.push(Number(text.substring(9)));
					}					
				}
			});
			if( nodeList.length > 0 ){
				fnComnNumberSort(nodeList);
				newNode = newNode + (Number(nodeList[nodeList.length-1])+1);	
			}else{
				newNode = newNode + "0";
			}
			
			
			if(!node) { node = { "text" : this.get_string(newNode) }; }
			if(typeof node === "string") {
				node = { "text" : node };
			} else {
				node = $.extend(true, {}, node);
			}
			if(node.text === undefined) { node.text = this.get_string(newNode); }
			var tmp, dpc, i, j;
